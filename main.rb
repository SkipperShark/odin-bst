...
class Tree
  ...
  def build_tree(array, node)
    p "node : #{node}, array : #{array}"
    return nil if array.empty? || array.nil?
    return Node.new(array.first, nil, nil) if array.length == 1

    midpoint = (array.length / 2)
    mid_ele = array[midpoint]
    return Node.new(mid_ele, build_tree(array[0...midpoint], node), build_tree(array[(midpoint + 1)..(array.length)], node))
    # build_tree(array[0...midpoint], node)
    # build_tree(array[(midpoint + 1)..(array.length)], node)
  end
